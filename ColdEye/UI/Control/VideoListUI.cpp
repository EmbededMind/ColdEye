...
void CVideoListUI::AddRecordFile(CRecordFileInfo* pInfo)
{
	...
	if (pItem){//与第一个节点属于同一天，添加到当前节点中
		if (pItem->Info->tBegin / (3600 * 24) == pInfo->tBegin/(3600*24)){ //refTime.GetTime() / (3600 * 24)){
			pNode = (CVideoListUI::Node*)pItem->GetTag();
			AddChildNode(tbegin.Format("%Y-%m-%d  %H:%M:%S") + _T("-") + tend.Format("%H:%M:%S"),pNode, 0, pInfo);
		}
			...
			if (pItem)
			{
				...
				pNode = (CVideoListUI::Node*)pItem->GetTag();
				pNode->data()._text = tbegin.Format("%Y-%m-%d  %H:%M:%S") + _T("-") + tend.Format("%H:%M:%S");
			}
		}
	}
	else //List中未添加任何文件
